...
>- \textbf{Rescaling}
>- \textbf{Shifting}
>- Rescaling and shifting are useful for \textbf{interpretation} of regression coefficients. 
...
summary(lm(hhincome~ownershp, data=msp_micro))
```
...
\tiny
```{r, echo=F}
ggplot(data=msp_micro[age %in% 25:64],
       aes(x=age,
           y=hhincome)) + 
  geom_point() + 
...
\tiny
```{r, echo=T}
msp_micro[, renter := ifelse(ownershp=='rented',1,0)]
...
\tiny
```{r, echo=F}
ggplot(data=msp_micro[!is.na(hhincome)],
       aes(x=renter,
           y=hhincome)) + 
  geom_point() + 
  geom_smooth(method='lm') + 
...
\tiny
```{r, echo=F}
ggplot(data=msp_micro[hhincome<=150000],
       aes(x=hhincome,
           y=renter)) + 
  geom_point() + 
  geom_smooth(method='lm') + 
